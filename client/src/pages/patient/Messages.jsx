...
// meant for the patient. Deliberately just type-and-send -- no category
// picker here, since asking a patient to classify their own message before
// they can talk to their pharmacist adds friction without helping them.
// The backend still accepts a category (used for staff-side triage), so
// every message here is just sent as 'general'.
export default function Messages() {
  ...
  async function send(e) {
    ...
    setError('');
    try {
      await apiFetch('/messages', { method: 'POST', body: JSON.stringify({ body: text, category: 'general' }) });
      setBody('');
      load();
    } catch (err) {
